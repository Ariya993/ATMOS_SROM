...
{
    public class SO_SHOWLESALES_DA
    {
            ...
            try
            {
                    ...
                    " from SO_WHOLESALES_HEADER {0}", where), Connection))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        item.MARGIN = reader.IsDBNull(reader.GetOrdinal("MARGIN")) ? 0 : reader.GetDecimal(9);
        ...
        public List<SO_WHOLESALES> GetSOToShPutusList(string where)
        {
            ...
            try
            {
                ...
                using (SqlCommand command = new SqlCommand(string.Format("select  SCAN_NO , NO_SO, QTY, QTY_REAL, TGL_REAL, TGL_KIRIM, ID, STATUS_HEADER from vw_SoScan {0} ORDER BY ID DESC", where), Connection))
                {
                    command.CommandType = CommandType.Text;
                    Connection.Open();
                    ...
                    while (reader.Read())
                    {
                        ...
                        item.STATUS_HEADER = reader.GetString(7);
                        listTemp.Add(item);
                    }
                    reader.Close();
                }
        ...
        public List<vw_CloseSO> GetCloseSOList(string where)
        {
            ...
            try
            {
                ...
                using (SqlCommand command = new SqlCommand(string.Format("select NO_PO, NO_SO, KODE_CUST, TGL_TRANS, SEND_DATE, QTY, QTY_REAL_1, TGL_KIRIM_1, QTY_REAL_2, TGL_KIRIM_2, FRETUR, NO_BON, STATUS_HEADER from VW_CLOSESO {0} ORDER BY NO_SO DESC", where), Connection))
                {
                    command.CommandType = CommandType.Text;
                    Connection.Open();
        ...
        public virtual void UpdateSoWholesalesHeaderStatus(string noSO, String Stat)
        {
            ...
            try
            {
                ...
                string query = String.Format("UPDATE SO_WHOLESALES_HEADER set STATUS_HEADER = @STATUS_HEADER where NO_SO = @noSO");
                Connection.Open();
                using (SqlCommand command = new SqlCommand(query, Connection))
                {
                    command.Parameters.Add("@noSO", SqlDbType.VarChar).Value = noSO;
                    command.Parameters.Add("@STATUS_HEADER", SqlDbType.VarChar).Value = Stat;
        ...
        public string SendSOInsToSHPutus(string uName, string tglfornoso, string NoScan, decimal totalprice)
        {
            string newId = "";
            SqlConnection Connection = new SqlConnection(conString);
            try
            {
                using (SqlCommand command = new SqlCommand("Usp_SendWholesale", Connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.Add("@user", uName);
                    command.Parameters.Add("@tglForNoSO", tglfornoso);
                    command.Parameters.Add("@NoScan", NoScan);
                    command.Parameters.Add("@totalPrice", totalprice);
                    command.CommandTimeout = 300;
                    Connection.Open();
        ...
        public virtual void UpdateSoWholesalesHeader(SO_WHOLESALES_HEADER item)
        {
            string newId = "Berhasil";
            SqlConnection Connection = new SqlConnection(conString);
            try
            {
                string query = String.Format("UPDATE SO_WHOLESALES_HEADER set NO_PO = @NO_PO, KODE_CUST = @KODE_CUST, " +
                    "KODE = @KODE, TGL_TRANS = @TGL_TRANS, SEND_DATE = @SEND_DATE, FRETUR = @FRETUR, MARGIN = @MARGIN, " +
                    "UPDATED_BY = @UPDATED_BY, UPDATED_DATE = GETDATE(), QTY = @QTY " +
                    "where NO_SO = @noSO AND ID = @ID");
                Connection.Open();
                using (SqlCommand command = new SqlCommand(query, Connection))
                {
                    command.Parameters.Add("@noSO", SqlDbType.VarChar).Value = item.NO_SO;
                    command.Parameters.Add("@ID", SqlDbType.BigInt).Value = item.ID;

                    command.Parameters.Add("@NO_PO", SqlDbType.VarChar).Value = item.NO_PO;
                    command.Parameters.Add("@KODE_CUST", SqlDbType.VarChar).Value = item.KODE_CUST;
                    command.Parameters.Add("@KODE", SqlDbType.VarChar).Value = item.KODE;
                    command.Parameters.Add("@TGL_TRANS", SqlDbType.Date).Value = item.TGL_TRANS;
                    command.Parameters.Add("@SEND_DATE", SqlDbType.Date).Value = item.SEND_DATE;
                    command.Parameters.Add("@FRETUR", SqlDbType.VarChar).Value = item.FRETUR;
                    command.Parameters.Add("@MARGIN", SqlDbType.Decimal).Value = item.MARGIN;
                    command.Parameters.Add("@UPDATED_BY", SqlDbType.VarChar).Value = item.UPDATED_BY;
                    command.Parameters.Add("@QTY", SqlDbType.Int).Value = item.QTY;
        ...
        public string UpdateSoWholesalesDetail(SO_WHOLESALES_HEADER SoHeader)
        {
            string newId = "";
            SqlConnection Connection = new SqlConnection(conString);
            try
            {
                using (SqlCommand command = new SqlCommand("Usp_UpdateSOWholesaleDetail", Connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.Add("@idheader", SoHeader.ID);
                    command.Parameters.Add("@NoSO", SoHeader.NO_SO);
                    command.Parameters.Add("@nama", SoHeader.UPDATED_BY);
                    command.Parameters.Add("@frtur", SoHeader.FRETUR);
                    command.CommandTimeout = 300;
                    Connection.Open();
        ...
        public virtual void CopySoDetailtoTempStruck(string Where)
        {
            string newId = "Berhasil";
            SqlConnection Connection = new SqlConnection(conString);
            try
            {
                string query = String.Format("INSERT INTO TEMP_STRUCK(ID_KDBRG, ID_SHOWROOM, BARCODE, ITEM_CODE, ART_DESC, " +
                    "WARNA, SIZE, BRAND, PRICE, ART_PRICE, BON_PRICE, SPCL_PRICE, DISCOUNT, JENIS_DISCOUNT, NET_PRICE, QTY, " +
                    "SA_NAME, ID_ACARA, NET_ACARA, CREATED_BY, CREATED_DATE, RETUR, MEMBER, SERVICE, EPC, KODE, SHOWROOM, " +
                    "DISC_TEMP) " +
                    "Select ID_KDBRG, ID_SHOWROOM, BARCODE, ITEM_CODE, ART_DESC, WARNA, SIZE, BRAND, PRICE, ART_PRICE, " +
                    "BON_PRICE, SPCL_PRICE, DISCOUNT, JENIS_DISCOUNT, NET_PRICE, QTY, SA_NAME, ID_ACARA, NET_ACARA, " +
                    "CREATED_BY, CREATED_DATE, RETUR, MEMBER, SERVICE, EPC, KODE, SHOWROOM, DISC_TEMP " +
                    "From SO_WHOLESALES {0}", Where);
                Connection.Open();
                using (SqlCommand command = new SqlCommand(query, Connection))
                {
                    //command.Parameters.Add("@noSO", SqlDbType.VarChar).Value = noSO;
